...
for j in range(1,9):
    for i in itertools.combinations(z, j):
        ...
        lr_kf=linear_model.LogisticRegression(C=1.0, penalty='l1', tol=1e-6)     # 定义一个逻辑回归模型
        x =trd[i]
        y =trd["Survived"]
        score=cross_val_score(lr_kf, x, y, cv=5)

        if (score.mean() > temp1 and score.std() < 0.5):
            temp0 = score
            temp1 = score.mean()
            temp2 = score.std()
            dict = {temp1: i}
...
# 逻辑回归进行预测
lr = linear_model.LogisticRegression(C=1.0, penalty='l1', tol=1e-6)  # 定义一个逻辑回归模型
x_trd = trd[c]
y_trd = trd["Survived"]
lr.fit(x_trd, y_trd)
x_tsd = tsd[c]
y_tsd = lr.predict(x_tsd)
result = pd.DataFrame({'PassengerId': tsd['PassengerId'].values, 'Survived': y_tsd.astype(np.int32)})
result.to_csv("../result/result_lr.csv", index=False)
