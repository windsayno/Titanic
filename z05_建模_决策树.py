...
for j in range(1,9):
    for i in itertools.combinations(z, j):
        ...
        stc_kf=DecisionTreeClassifier()         # 定义一个决策树分类器
        x =trd[i]
        y =trd["Survived"]
        score=cross_val_score(stc_kf, x, y, cv=5)
...
# 决策树建模
dtc = DecisionTreeClassifier()  # 定义一个决策树分类器
x_trd = trd[c]
y_trd = trd["Survived"]
dtc.fit(x_trd, y_trd)  # 进行建模
x_tsd = tsd[c]
y_tsd = dtc.predict(x_tsd)  # 进行预测
result = pd.DataFrame({'PassengerId': tsd['PassengerId'].values, 'Survived': y_tsd.astype(np.int32)})  # 更改预测结果格式
result.to_csv("../result/result_dtc.csv", index=False)  # 输出结果